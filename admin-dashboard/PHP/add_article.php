...
if (isset($_POST['blog'])) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["hero-image"]["tmp_name"], $target_file)) {
            ...
            $query = "INSERT INTO `articles` (`visibility`,`title`,`comments`,`slug`,`date_of_posting`, `time_of_posting`,`hero_image`, `excerpt`, `content`, `type`, `author_id`, `created_at`, `updated_at`, `from_ip`, `from_browser`, `hero_image_alt`,`status` , `article_id`,`keywords`,`meta_title`,`meta_url`,`meta_description`,`meta_image`) VALUES ('$visibility','$title','$comments','$slug','$date_of_posting', '$time_of_posting', '$hero_image', '$excerpt', '$content', '$type', '$author_id', '$date_now', '$date_now', '$from_ip', '$from_browser', '$hero_image_alt', '$status', '$article_id', '$keywords','$meta_title','$url','$meta_description','$meta_image')";
